...
class App extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      filteredImage: imagesList.filter(item => item.category === 'FRUIT'),
    }
  }
  ...
  onChangeDisplayText = tabId => {
    const filteredImage = imagesList.filter(item => item.category === tabId)
    this.setState({filteredImage})
  }

  onChangeImageId = id => {
    const {randomImage, gateStatus} = this.state
    if (randomImage.id === id) {
      this.setState(prevState => ({count: prevState.count + 1}))
    } else {
      clearInterval(this.timerId)
      this.setState(prevState => ({gateStatus: !prevState.gateStatus}))
    }

    const lengthOfList = imagesList.length
    const randomnumImage = Math.floor(Math.random() * lengthOfList)
    const getRandomImage = imagesList[randomnumImage]

    this.setState({randomImage: getRandomImage})
  }

  render() {
    const {gateStatus, count, timer, filteredImage, randomImage} = this.state
    ...
    return (
      <div className="main-contaier">
        <nav className="nav-container">
          ...
          <nav className="timer-container">
            <nav className="Score-container">
              <p className="para">
                Score: <span className="span-element">{count} </span>
              </p>
            </nav>
            <nav className="timer-section">
              <img
                className="timer-img"
                alt="timer"
              ...
              />
              <p>
                <span className="timer-element">{timer} sec</span>
              </p>
            </nav>
          </nav>
        </nav>
        {gateStatus ? (
          <div className="end-game">
            <div className="gameTermination">
              ...
              <button onClick={this.onResetGame} className="play-Again">
                <img
          ...
          <div className="image-card">
            <div className="randomImage-container">
              <img className="random-image" src={randomImage.imageUrl} />
            </div>
            <div className="buttons-container">
              {tabsList.map(items => (
                <li key={items.tabId}>
                  <button
                    onClick={() => this.onChangeDisplayText(items.tabId)}
                    className="butons-items"
                  >
                    {items.displayText}
                  </button>
                </li>
            ...
            <div className="list-contaier">
              {filteredImage.map(items => (
                <ul>
                  <li className="button-img-container" key={items.id}>
                    <button
                      onClick={() => this.onChangeImageId(items.id)}
                      className="button-items"
                    >
                      <img className="images" src={items.thumbnailUrl} />
                    </button>
                  </li>
                </ul>
              ))}
